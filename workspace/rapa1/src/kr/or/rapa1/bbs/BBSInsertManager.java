...
public class BBSInsertManager extends HttpServlet {	
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			String sql = String.format("insert into bbsmanager (title, odr, useflag, grp, head, rlevel, wlevel, line)"
					+"values('%s','%s','%s','%s','%s','%s','%s','%s')", title, odr, useflag, grp, head, rlevel, wlevel, line);			
			...
			{  // 1보다 작으면 error
				...
				request.setAttribute("nextURL", "main.jsp?CMD=manBBS.jsp?sub=2");
